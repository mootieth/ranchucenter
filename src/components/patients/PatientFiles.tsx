...
import { Upload, Trash2, Download, Loader2, File, ChevronLeft, ChevronRight } from "lucide-react";
...
const PatientFiles = ({ patientId }: PatientFilesProps) => {
  ...
  const processFiles = async (fileList: FileList | File[]) => {
    ...
    try {
      ...
      for (const file of selectedFiles) {
        ...
        const fileName = `patients/${patientId}/${Date.now()}-${Math.random().toString(36).substring(7)}.${fileExt}`;
  ...
  const handleDrop = (e: React.DragEvent) => { e.preventDefault(); e.stopPropagation(); setIsDragging(false); if (e.dataTransfer.files) processFiles(e.dataTransfer.files); };
  ...
  const goNext = () => { if (previewIndex !== null && previewIndex < previewableFiles.length - 1) setPreviewIndex(previewIndex + 1); };
  const goPrev = () => { if (previewIndex !== null && previewIndex > 0) setPreviewIndex(previewIndex - 1); };
  ...
  return (
    <div className="space-y-3">
      ...
      {files.length > 0 ? (
          ...
          {files.map((file) => (
            ...
            >
              {isImage(file.file_type) ? (
                <img src={file.file_url} alt={file.file_name} className="w-10 h-10 rounded object-cover border shrink-0" />
              ) : (
                <div className="w-10 h-10 rounded bg-muted flex items-center justify-center shrink-0">
                  <File className={`w-4 h-4 ${isPdf(file.file_type) ? "text-destructive" : "text-muted-foreground"}`} />
                </div>
              )}
              ...
              {isAdmin && (
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
                  ...
                  <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => deleteMutation.mutate(file)}>
        ...
        <p className="text-sm text-muted-foreground text-center py-4">ยังไม่มีเอกสารทั่วไป</p>
      )}
      ...
      <Dialog open={previewIndex !== null} onOpenChange={(open) => !open && setPreviewIndex(null)}>
        <DialogContent className="max-w-4xl max-h-[90vh] p-0 gap-0 overflow-hidden">
          ...
          <div className="flex items-center justify-between p-3 border-t bg-muted/30">
            <Button variant="outline" size="sm" onClick={goPrev} disabled={previewIndex === 0}>
            ...
            <Button variant="outline" size="sm" onClick={goNext} disabled={previewIndex === previewableFiles.length - 1}>
              ถัดไป
